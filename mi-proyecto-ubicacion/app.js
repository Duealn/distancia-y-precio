...
const costoPorKm = 0.50; // precio por km
let userCircle;
...
// Ahora la ruta va desde la TIENDA hasta el USUARIO
async function getDrivingDistance(origenTienda, destinoUsuario) {
  const request = {
    origin: origenTienda,         // origen: tienda
    destination: destinoUsuario,  // destino: usuario
...
document.getElementById("btn").onclick = async () => {
  ...
  navigator.geolocation.getCurrentPosition(async (pos) => {
    ...
    for (const tienda of stores) {
      ...
      try {
        // ahora la ruta es desde la tienda hacia el usuario
        const { distanciaKm, ruta } = await getDrivingDistance(tiendaLoc, userLoc);
    ...
    if (rutasCercanas.length === 0) {
      ...
      lista.innerHTML = rutasCercanas
        .map(r => {
          const costo = Math.ceil(r.distanciaKm * costoPorKm);
          return `<li>${r.tienda.name} → Usuario — ${r.distanciaKm.toFixed(2)} km — $${costo}</li>`;
        })
        .join("");
